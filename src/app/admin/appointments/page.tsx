  ...
  const [{ data: requests, error }, { data: doctors, error: doctorError }] =
    await Promise.all([
      db
        ...
        .select(
          "*,patient:profiles!appointment_requests_patient_id_fkey(full_name,email,phone),requested:doctors!appointment_requests_requested_doctor_id_fkey(id,full_name,specialization,location,hospital_name),appointment_request_candidates(preference_rank,doctor:doctors(id,full_name,location,hospital_name,specialization))",
        )
  ...
  return (
    <>
      <Heading eyebrow="ADMIN QUEUE" title="Appointment requests">
        See exactly which doctor the patient selected, their symptoms / reason for
        visit, then confirm the final doctor and appointment details.
      </Heading>
      ...
      <div className="stack">
        {requests?.map((request) => {
          ...
          const selectedDoctor = request.requested;
          const selectedDoctorServesArea = selectedDoctor
            ? matches(selectedDoctor.location, request.area)
            : false;
            ...
            >
              ...
              {selectedDoctor && !selectedDoctorServesArea && (
                <p className="muted">
                  ...
                  to serve {request.area}. Assign a suitable local doctor below if
                  needed.
                </p>
              )}
